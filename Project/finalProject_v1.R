...
glass <- Glass
beans <- read.csv(
  "C:/Users/Maggie/Downloads/Spring2025/MATH252/project/bean.csv",
  header = TRUE,
...
for (nm in names(glass_num_all)) {
  boxplot(glass_num_all[[nm]], main = paste0(nm, "\n(", 
                                             round(glass_outlier_metrics$outlier_percentages[nm], 1), "% outliers)"), 
...
glass_x <- scale(glass_num_all)
beans_x <- scale(beans_num_all)

# Extract true labels
glass_labels <- as.numeric(glass$Type)
beans_labels <- as.numeric(as.factor(beans$Class))

datasets <- list(
  Glass = list(x = glass_x, labels = glass_labels, 
               outlier_density = glass_outlier_metrics$overall_outlier_density),
  Beans = list(x = beans_x, labels = beans_labels, 
               outlier_density = beans_outlier_metrics$overall_outlier_density)
)
