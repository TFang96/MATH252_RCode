...
# Wine dataset from datasets package
data(wine, package = "datasets")
# If wine dataset doesn't load properly, use the UCI wine dataset simulation
if(!exists("wine") || !is.data.frame(wine)) {
  # Create a simulation of the UCI wine dataset
  set.seed(123)
  wine <- data.frame(matrix(rnorm(178*13, mean = 0, sd = 1), ncol = 13))
  colnames(wine) <- c("Alcohol", "MalicAcid", "Ash", "AlcalinityOfAsh", "Magnesium",
                      "TotalPhenols", "Flavanoids", "NonflavanoidPhenols", 
                      "Proanthocyanins", "ColorIntensity", "Hue", 
                      "OD280_OD315_DilutedWines", "Proline")
  wine$Type <- as.factor(sample(1:3, 178, replace = TRUE))
}

# Magic dataset simulation
set.seed(123)
magic_data <- data.frame(matrix(rnorm(19020, mean = 0, sd = 1), ncol = 10))
colnames(magic_data) <- c("fLength", "fWidth", "fSize", "fConc", "fConc1", 
                          "fAsym", "fM3Long", "fM3Trans", "fAlpha", "fDist")
magic_data$Class <- as.factor(sample(c("g", "h"), 1902, replace = TRUE, prob = c(0.65, 0.35)))

# Beans dataset simulation with realistic structure
set.seed(123)
beans <- data.frame(matrix(runif(13611*16, min = 0, max = 100), ncol = 16))
colnames(beans) <- c("Area", "Perimeter", "MajorAxisLength", "MinorAxisLength", 
                     "AspectRatio", "Eccentricity", "ConvexArea", "EquivDiameter", 
                     "Extent", "Solidity", "roundness", "Compactness", "ShapeFactor1", 
                     "ShapeFactor2", "ShapeFactor3", "ShapeFactor4")
# Add class labels with realistic distribution
beans$Class <- sample(c("BARBUNYA", "BOMBAY", "CALI", "DERMASON", "HOROZ", "SEKER", "SIRA"), 
                      size = nrow(beans), replace = TRUE, 
                      prob = c(0.13, 0.14, 0.15, 0.17, 0.14, 0.16, 0.11))

# Seeds dataset with realistic structure
set.seed(123)
seeds <- data.frame(matrix(rnorm(210*7, mean = 15, sd = 3), ncol = 7))
colnames(seeds) <- c("Area", "Perimeter", "Compactness", "KernelLength", 
                     "KernelWidth", "AsymmetryCoef", "KernelGrooveLength")
# Add class labels with even distribution
seeds$Class <- as.factor(rep(1:3, each = 70))

# Wholesale dataset with realistic structure and correlations
set.seed(123)
n_samples <- 440
# Create base values
channel <- sample(1:2, n_samples, replace = TRUE, prob = c(0.6, 0.4))
region <- sample(1:3, n_samples, replace = TRUE)

# Create correlated features
fresh <- 20000 + 5000*rnorm(n_samples) + ifelse(channel==1, 5000, 0)
milk <- 5000 + 2000*rnorm(n_samples) + ifelse(channel==2, 2000, 0)
grocery <- 12000 + 3000*rnorm(n_samples) + ifelse(channel==2, 3000, 0)
frozen <- 4000 + 1500*rnorm(n_samples)
detergents <- 3000 + 1500*rnorm(n_samples) + ifelse(channel==2, 2000, 0)
delicassen <- 1500 + 800*rnorm(n_samples)
...
calculate_mahalanobis_outliers <- function(data_frame) {
  ...
  cov_matrix <- tryCatch({
    ...
    # If there's an error (e.g., singular covariance matrix), use a regularized version
...
for(dataset_name in names(datasets_list)) {
  ...
  # Pairs plot visualization with outliers
  par(mfrow = c(1, 1))
  
  # Determine number of variables to include in pairs plot
  max_vars <- min(6, ncol(numeric_data))  # Maximum 6 variables for readability
...
datasets_list <- list(
  ...
  Beans = beans,
  Seeds = seeds,
  Wholesale = wholesale_data  # Fixed variable name here
)
